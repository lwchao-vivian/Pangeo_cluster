...
# lagged covariance routines
lag1cov = lambda a1: (numpy.average(a1[1:]*a1[:-1])/numpy.std(a1)**2)
...
class ols:
    ...
    edited by AED 131227 to include autoregression in the time series
    """
    ...
    def estimate(self):
        ...
        self.inv_xx = inv(numpy.array(dot(self.x.T,self.x)))
        ...
        self.df_e = self.nobs - self.ncoef              # degrees of freedom, error // aed
    ...
    def est_auto(self): # statistics adjusting for autocorrelation 
        ...
        # from Santer, calculated adjusted DOFs
        # adjx = factor that reduces # of obs. to # of independent obs.
        adjx=lag1cov(self.e);adjx=(1-adjx)/(1+adjx)
        ...
        # conf95 is the 95% confidence interval based on 
        # the adjusted number of DOF
        self.conf95=t(self.df_e-1).isf(0.025)*self.se
        self.conf90=t(self.df_e-1).isf(0.05)*self.se
        self.conf68=t(self.df_e-1).isf(0.16)*self.se
